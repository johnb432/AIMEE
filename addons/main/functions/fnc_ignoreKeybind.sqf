...
!(GVAR(settingHide) && {!isNil {
    ...
    if (_this isEqualTo "engineControlACE") then {
        if (((["ACE3 Vehicles", "ace_weaponselect_EngineOn"] call CBA_fnc_getKeybind) select 5 select 0) isEqualTo -1 || {((["ACE3 Vehicles", "ace_weaponselect_EngineOff"] call CBA_fnc_getKeybind) select 5 select 0) isEqualTo -1}) then {
            nil;
        } else {
            "aceEngineKeybinds";
        };
