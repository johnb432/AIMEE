...
private _submenu_for_turret = {
	...
	[[
		format [QUOTE(CLAS(turret_%1_action)), _turret_path],
		_text,
		_icon,
...
if (count _drivers != 0 && { [_player, _vehicle, _drivers#0] call FUNC(can_switch) }) then {
...
if (count _commanders != 0 && { [_player, _vehicle, _commanders#0] call FUNC(can_switch) }) then {
	_submenus pushBack ([_commanders#0, ICON_COMMANDER] call _submenu_for_turret);
};
if (count _gunners != 0 && { [_player, _vehicle, _gunners#0] call FUNC(can_switch) }) then {
	_submenus pushBack ([_gunners#0, ICON_GUNNER] call _submenu_for_turret);
};
{
	if ([_player, _vehicle, _x] call FUNC(can_switch)) then {
		_submenus pushBack ([_x, ICON_GUNNER] call _submenu_for_turret);
	};
} forEach _turrets;
{
	if ([_player, _vehicle, _x] call FUNC(can_switch)) exitWith {
		_submenus pushBack [[
			QUOTE(CLAS(cargo_action)),
			TEXT_CARGO,
			ICON_CARGO,
			{ _player action _this#2 },
			{ true },
